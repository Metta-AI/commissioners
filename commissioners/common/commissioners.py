        ...
        return DivisionLeaderboardsSnapshot(
            views=[
                DivisionLeaderboardViewSnapshot(
                    ...
                    title="Score",
                    axes=[
                        DivisionLeaderboardAxisSnapshot(key="rank", label="Rank", value_type="integer", sort="asc"),
                        DivisionLeaderboardAxisSnapshot(key="score", label="Score", value_type="number", sort="desc"),
                        DivisionLeaderboardAxisSnapshot(
                            key="rounds_played",
                            label="Rounds Played",
                            value_type="integer",
...
class BaselineCommissioner(Commissioner):
    ...
    def _rank_division_view_by_metric(
        ...
        *,
        ...
        axes = [
            DivisionLeaderboardAxisSnapshot(key="rank", label="Rank", value_type="integer", sort="asc"),
            DivisionLeaderboardAxisSnapshot(key=score_key, label=score_axis_label, value_type="number", sort="desc"),
            DivisionLeaderboardAxisSnapshot(key="rounds_played", label="Rounds Played", value_type="integer"),
        ]
    ...
    def _rank_division_leaderboards_from_config(
        ...
        *,
        ...
        for raw_view in leaderboard_configs:
            raw_axes = raw_view.get("axes")
            axes_config: list[Any] = raw_axes if isinstance(raw_axes, list) else []
            configured_score_axis = next(
                (axis for axis in axes_config if isinstance(axis, dict) and axis.get("sort") == "desc"),
                None,
            )
            score_key = str(
            ...
            if axes_config:
                view.axes = [DivisionLeaderboardAxisSnapshot.model_validate(axis) for axis in axes_config]
            views.append(view)

        if not any(view.key == default_view_key for view in views) and views:
            default_view_key = views[0].key
        return DivisionLeaderboardsSnapshot(default_view_key=default_view_key, views=views)
